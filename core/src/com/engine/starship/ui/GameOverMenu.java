...
public class GameOverMenu extends MenuObject {
    ...
    public void show() {
        ...
        root.add(scoreBackground).expand().bottom().width(region.originalWidth * 7).height(region.originalHeight * 6);
        root.row();
    ...
    private void renderScore(Batch batch){
        //Renders score.
        font.draw(batch, String.valueOf(manager.universeManager.getScoreCounter()),(camera.viewportWidth/1.6f),(camera.viewportHeight/1.8f) + 120 + region.originalHeight);
        //Renders kills.
        font.draw(batch, String.valueOf(manager.universeManager.getScoreCounter()),(camera.viewportWidth/1.6f),(camera.viewportHeight/1.8f) + 75 + region.originalHeight);
        //Renders gems obtain.
        font.draw(batch, String.valueOf(manager.universeManager.getCollectedGemsCounter()),(camera.viewportWidth/1.6f),(camera.viewportHeight/1.8f) + 25 + region.originalHeight);
        //Renders only if high score.
        if (manager.universeManager.scoreParser.isNewHighScore()){
            highScore.draw(batch,(camera.viewportWidth/2.0f),(camera.viewportHeight/1.8f) +  200);
        }
    }
