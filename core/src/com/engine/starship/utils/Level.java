...
import com.engine.starship.utils.entities.CTypeAsteroid;
import com.engine.starship.utils.entities.NomadAsteroid;
import com.engine.starship.utils.entities.RogueAsteroid;
import com.engine.starship.utils.entities.STypeAsteroid;
import com.engine.starship.utils.entities.SalusAsteroid;
import com.engine.starship.utils.entities.TankAlienStarship;
...
public class Level {
    ...
    protected void spawnAsteroids() {
        ...
        int newY = RANDOM_XS_128.nextInt((int) manager.cameraUtils.viewportHeight - 3);

        Asteroid asteroid;
        //Random checks whether to spawn a health asteroid.
        if (RANDOM_XS_128.nextBoolean()) {
            asteroid = new SalusAsteroid(newX, newY);
        } else if (RANDOM_XS_128.nextBoolean()) {
            asteroid = new RogueAsteroid(newX, newY);
        } else if (RANDOM_XS_128.nextBoolean()) {
            asteroid = new NomadAsteroid(newX, newY);
        }  else if (RANDOM_XS_128.nextBoolean()) {
            asteroid = new CTypeAsteroid(newX, newY);
        } else if (RANDOM_XS_128.nextBoolean()) {
            asteroid = new STypeAsteroid(newX, newY);
        }else {
            asteroid = new Asteroid(newX, newY);
        }
